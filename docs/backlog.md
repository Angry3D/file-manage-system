...
| TODO-0007 | done | bug | P0 修复管理端删除图片接口无成功响应。 | 项目通读 | `server/src/controller/admin/image.js` 的 `deleteAction` 删除后没有返回 `this.success()`，管理端删除流程依赖 Promise 完成后刷新列表；现已返回删除行数。 | 已完成；等待用户确认或明确要求后再归档。 |
| TODO-0008 | done | bug | P0 修复编辑图片时缩略图和上传图片 ID 不同步更新。 | 项目通读 | `web-admin/src/views/file/image/ImageAdd.vue` 编辑提交了 `image_thumb`，但 `server/src/controller/admin/image.js` 的 `setAction` 未更新 `image_thumb` 和 `image_id`；现已修复前端字段映射并更新服务端字段。 | 已完成；等待用户确认或明确要求后再归档。 |
| TODO-0009 | done | bug | P0 修复新增多张图片时未等待所有数据库写入完成。 | 项目通读 | `server/src/controller/admin/image.js` 的 `addAction` 使用 `params.images.forEach(async ...)` 后立即返回成功；现已改为等待所有写入完成后返回实际写入数量。 | 已完成；等待用户确认或明确要求后再归档。 |
...
| TODO-0011 | done | chore | P1 服务端配置安全治理：外置敏感配置和环境差异配置。 | 项目通读 | `server/src/config/adapter.js` 存在明文数据库连接信息；`server/src/config/define.js` 和 `server/script/file-server.sh` 存在本机绝对路径和固定域名；现已支持环境变量覆盖并移除数据库密码明文默认值。 | 已完成；数据库密码需通过 `BABYLIFE_DB_PASSWORD` 注入，等待用户确认或明确要求后再归档。 |
| TODO-0012 | done | bug | P1 增强图片上传链路健壮性。 | 项目通读 | `server/src/controller/common/upload.js` 写文件前未确保目标目录存在，也没有统一捕获 Sharp 或文件系统错误；现已补充目录创建、Sharp/文件系统错误捕获和 GIF 流写入错误处理。 | 已完成；等待用户确认或明确要求后再归档。 |
| TODO-0013 | done | bug | P2 修复前端已有体验和状态问题。 | 项目通读 | `Login.vue` 的用户名表单 `prop` 拼写为 `accout`；`XTable.vue` 请求失败时 loading 可能不恢复；`web-h5/src/components/XList.vue` 存在 `defautl` 拼写和分页完成判断问题；`ImageView.vue` 边界提示与循环行为不一致；现已修复并移除图片管理调试输出。 | 已完成；等待用户确认或明确要求后再归档。 |
| TODO-0014 | done | chore | P2 建立最小验证体系，保证后续修复和依赖升级可验证。 | 项目通读；本地命令检查 | 现有脚本包含服务端 `test/lint`、前端 `lint/build`，但尚未确认当前环境可跑；现已新增根目录 `verify` 系列脚本和 `docs/verification.md`。 | 已完成；`pnpm run verify` 已通过，等待用户确认或明确要求后再归档。 |
